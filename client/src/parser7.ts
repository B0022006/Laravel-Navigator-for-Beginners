...
export async function parse(projectPath: string): Promise<void> {
    ...
    function analyzePHP(content: string, filePath: string): AnalysisResult {
        ...
        function getMethodChain(node: any): { methods: string[], arguments: any[] } {
            ...
            while (currentNode.kind === 'call') {
                if (currentNode.what.kind === 'propertylookup' || currentNode.what.kind === 'staticlookup') {
                    let methodName = '';
                    if (currentNode.what.offset.kind === 'identifier') {
                        methodName = currentNode.what.offset.name;
                    } else if (currentNode.what.offset.kind === 'constref') {
                        methodName = currentNode.what.offset.name;
                    }
                    methods.push(methodName);
                    argumentsList.push(currentNode.arguments);

                    currentNode = currentNode.what.what;
                } else if (currentNode.what.kind === 'identifier' || currentNode.what.kind === 'name') {
                    let methodName = currentNode.what.name;
                    methods.push(methodName);
                    argumentsList.push(currentNode.arguments);
                    break;
                } else {
